...
class TemplateFieldService {
    ...
    def List getUsedListEntries( TemplateField templateField ) {
        ...
        if (templateField.type == TemplateFieldType.STRINGLIST) {
            query = "SELECT DISTINCT y.templatefieldlistitemname FROM ${templateField.entity.simpleName.toLowerCase().replaceAll(/\B[A-Z]/) { '_' + it }.toLowerCase()}_template_string_list_fields x, template_field_list_item y WHERE x.template_string_list_fields_idx = '${templateField.name}' AND x.template_field_list_item_id = y.id;"
        }
        else if(templateField.type == TemplateFieldType.EXTENDABLESTRINGLIST) {
            query = "SELECT DISTINCT y.templatefieldlistitemname FROM ${templateField.entity.simpleName.toLowerCase().replaceAll(/\B[A-Z]/) { '_' + it }.toLowerCase()}_template_extendable_string_list_fields x, template_field_list_item y WHERE x._extendable_string_list_fields_idx = '${templateField.name}' AND x._extendable_string_list_fields_idx = y.id;"
        }
        else {
            return []
        }
