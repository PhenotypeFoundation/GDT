...
class TemplateEditorController {
	...
	def createField = {
		...
		if (TemplateField.find(new nl.grails.plugins.gdt.TemplateField(uniqueParams))) {
			response.status = 500;
		...
		if (params.type.toString() == 'STRINGLIST') {
			...
			params.listEntries.eachLine {
				...
				def listitem = new nl.grails.plugins.gdt.TemplateFieldListItem(name: name)
		...
		def templateField = new nl.grails.plugins.gdt.TemplateField(params);
		...
		if (params.type.toString() == 'STRINGLIST' || (templateField.type == TemplateFieldType.STRINGLIST && params.is_disabled)) {
			...
			if (params.listEntries) {
				...
				listEntryLines.eachLine {
					...
					if (!listitem) {
						listitem = new nl.grails.plugins.gdt.TemplateFieldListItem(name: name)
					}
